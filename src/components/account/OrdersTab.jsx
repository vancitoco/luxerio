  ...
  if (!orders.length) {
    return (
      <div className="flex flex-col items-center gap-4 py-20 text-center">
        <span className="inline-block border border-hairline px-3 py-1 font-display text-[9px] font-black uppercase tracking-widest text-secondary">
          No Orders
        </span>
      ...
      {orders.map((order) => {
        ...
        return (
          <div key={order.id} className="border-b border-hairline">
            ...
            >
              <span className="font-display text-sm font-black uppercase tracking-tight text-primary">
                #{order.orderNumber}
              </span>
              ...
              <span
                className={`hidden px-2 py-0.5 font-display text-[9px] font-black uppercase tracking-widest md:inline-block ${
                  STATUS_STYLES[status] ?? 'text-secondary'
                }`}
              >
                {status.replace('_', ' ')}
              </span>
              <span className="hidden font-display text-sm font-black tabular-nums text-primary md:block">
                ...
                {lines.map((line, i) => (
                  <div key={i} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-display text-xs font-black uppercase leading-tight tracking-tight text-primary">
                        {line.title}
                      </p>
                    ...
                    <div className="text-right shrink-0">
                      <p className="font-display text-xs font-black tabular-nums text-primary">
                        {line.variant?.price
